...
export const BarberCard = ({ id, name, photoUrl, specialty, experience, bio, isCompact, isSelected, onClick, isAdmin }) => {
      ...
      <div className={cn("flex-1", isCompact ? "" : "flex flex-col flex-1")}>
        ...
        {isAdmin && (
          <div className="mt-auto pt-4 border-t border-border/50 flex gap-2">
            <button className="flex-1 py-2 text-sm text-navy hover:bg-muted rounded-md transition-colors font-medium">Edit</button>
            <button className="flex-1 py-2 text-sm text-error hover:bg-error/10 rounded-md transition-colors font-medium">Deactivate</button>
          </div>
        )}
      </div>
    </CardWrapper>
